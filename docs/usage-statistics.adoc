...
. When there is a free slot for request Selenwright decides whether a Docker container or standalone driver process should be created. All requests during startup are marked as `pending`. Before proceeding to next step Selenwright waits for required port to be open. This is done by sending HEAD requests to the port.
. When a container or driver is started (ping is successful) Selenwright does a new session request just in the same way as standard Selenium client.
...
To send Selenwright statistics described in previous section you can use https://github.com/influxdata/telegraf[Telegraf]. For example to send status to https://github.com/graphite-project[Graphite]:
...
<4> this is localhost only if Telegraf is run on Selenwright host
...
+
```
one_min.selenwright_example_com.httpjson_selenwright.pending
one_min.selenwright_example_com.httpjson_selenwright.queued
one_min.selenwright_example_com.httpjson_selenwright.total
```
