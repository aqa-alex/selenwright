...
+
```
$ docker run -it --rm --entrypoint /usr/bin/ffmpeg selenwright/video-recorder -codecs
```
...
Selenwright is using `selenoid:options` key to read protocol extension capabilities:
