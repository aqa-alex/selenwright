...
Selenwright can improve some automation aspects with custom capabilities.
You can pass it in tests to enable/disable some features.


=== Live Browser Screen: enableVNC

Selenwright supports showing browser screen during test execution.
...
Then launch https://github.com/aqa-alex/selenwright-ui[Selenwright UI] to see the screen.
...
+
```
$ docker run -it --rm --entrypoint /usr/bin/ffmpeg selenwright-video-recorder:latest -codecs
```
...
When working with Selenwright you can set test case name by passing the following capability:
...
To achieve this with Selenwright use:
...
Without this capability launched browser containers will have the same timezone as Selenwright one.

=== Per-session Container Hostname: containerHostname
...
NOTE: Under `-caps-policy=strict` (default), the `env`, `dnsServers`, `hostsEntries`, `additionalNetworks`, and `applicationContainers` capabilities are restricted to admin users. See <<Authentication and Authorization>> for details.
