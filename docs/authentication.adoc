...
Log into the UI as an admin, open **Settings → API Tokens**, click **Create**, pick the owner and a human-readable name (e.g. `laptop-alice`, `ci-runner`). Copy the plaintext from the modal — it will not be shown again.

The same operation is exposed on the REST surface (admin-only, authenticated with a session cookie from the UI login or with an admin's own bearer token):
...
==== Env-var Seeding and the Dev Fallback

For a local dev loop you usually want to skip htpasswd entirely:
...
Lost it? Delete `<state-dir>/auth/tokens.json` and restart — a fresh token is printed.

In CI, pin the token instead via `SELENWRIGHT_AUTH_TOKEN`. On first boot (empty token store) selenwright hashes the env value and stores it as `owner=admin name=env-seed`. Subsequent boots that see the same env value are a no-op. If the env value does not match what is already stored, selenwright logs a warning and keeps the stored tokens intact — wipe `tokens.json` if you really want to re-seed.
