...
By default Docker container logs are saved to host machine hard drive. When using Selenwright for local development that's ok.
...
An optional Selenwright logging configuration file allows to specify which logging driver to use globally for all started Docker
